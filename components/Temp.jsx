...
const Temp = () => {
  ...
  const convertTemperature = () => {
    ...
    if (inputType === "Celsius") {
      ...
      setConvertedTemperature(
        // `${temperature} °C is ${convertedTemp.toFixed(2)} °F`
        `${convertedTemp.toFixed(2)} °F`
      );
    } else if (inputType === "Fahrenheit") {
      const convertedTemp = ((temperature - 32) * 5) / 9;
      setConvertedTemperature(
        // `${temperature} °F is ${convertedTemp.toFixed(2)} °C`
        `${convertedTemp.toFixed(2)} °C`
      );
    } else {
  ...
  return (
    <div className="container mx-0 lg:mx-60 p-10 md:p-20 border shadow-lg bg-transparent backdrop-blur-sm">
      ...
      <div className="mb-4">
        <label className=" mb-2 flex flex-col gap-2 items-center justify-center">
          {/* Enter Temperature: */}
          <input
            type="text"
      ...
      <div className="mb-4">
        <label className="mb-2 flex gap-2 items-center justify-center">
          <select
            value={inputType}
      ...
      <div>
        <p className="text-xl font-bold text-center bg-sky-200 p-4 mb-4">
